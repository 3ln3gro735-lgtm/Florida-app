...
RUTA_CSV = 'loteria_florida.csv' 

# --- CONFIGURACIÓN DE LA PÁGINA ---
st.set_page_config(
    # CAMBIO: Título y nombre de la aplicación
    page_title="Florida - Análisis Fusionado",
    page_icon="🌴",
    layout="wide"
)

# CAMBIO: Título y descripción principal
st.title("🌴 Florida - Análisis con Estado Actualizado")
st.markdown("Sistema de Predicción que calcula el estado **ACTUAL** de los números y dígitos para los sorteos de Tarde y Noche.")
st.info("ℹ️ **Importante:** Los estados se calculan comparando el salto actual de cada elemento con su propio promedio histórico de ausencia (redondeado). **El cálculo del salto ahora considera el sorteo específico (T/N) para mayor precisión.**")
...
def get_full_state_dataframe(df_historial, fecha_referencia):
    ...
    # Esto permite diferenciar entre los sorteos del mismo día (T, N)
    if 'Tipo_Sorteo' in df_historial_filtrado.columns:
        # CAMBIO: Mapa de orden de sorteos actualizado para solo T y N
        draw_order_map = {'T': 0, 'N': 1}
        # Rellenar valores nulos con un número alto para que se ordenen al final
        df_historial_filtrado['draw_order'] = df_historial_filtrado['Tipo_Sorteo'].map(draw_order_map).fillna(2)
...
def cargar_datos_florida(_ruta_csv, debug_mode=False):
    try:
        ...
        if not os.path.exists(ruta_csv_absoluta):
            ...
            st.warning("💡 **Solución:** Asegúrate de que el archivo 'loteria_florida.csv' exista en el mismo directorio que la aplicación.")
            st.stop()
        ...
        col_sorteo_name = next((col for col in df_historial.columns if df_historial[col].dropna().isin(['T', 'N']).any()), None)
        rename_map = {
            'Fecha': 'Fecha', 
        ...
        if 'Tipo_Sorteo' in df_historial.columns:
            draw_order_map = {'T': 0, 'N': 1}
            df_historial['draw_order'] = df_historial['Tipo_Sorteo'].map(draw_order_map).fillna(2)
...
def main():
    ...
    df_historial = cargar_datos_florida(RUTA_CSV, debug_mode)

    if df_historial is not None:
        # --- INFORMACIÓN DE ÚLTIMOS SORTEOS (TARDE Y NOCHE) ---
        st.sidebar.markdown("---")
        if 'Tipo_Sorteo' in df_historial.columns:
            # CAMBIO: Eliminada la sección de "Mañana" y ajustados los mensajes
